...
import runConfetti from '../../Testing/confetti.js';
...
function PricingCard({ title, price, time, description, planDetails, benefits, id, benefitTitle, highlightBorder ,onClose ,fetchSubscriptions}) {
    ...
    const createCheckoutSession = async (data) => {
        try {
            let config = {
                ...
                url: `${baseUrl}/v1/sub/create-checkout-session`,
                headers: {
    ...
    const getSubscription = async () => {
        try {
            const response = await axios.post(`${baseUrl}/v1/sub/get`, null, {
                headers: {
                    'Accept': 'application/json',
                    'Authorization': 'Bearer ' + userToken
                },
    ...
    return (
        <div className={`bg-gray-700 bg-opacity-10 dark:bg-opacity-30 rounded-3xl shadow-lg text-center p-6 border border-gray-500 dark:border-gray-400 ${highlightBorder ? 'dark:border-indigo-700 border border-dashed border-opacity-50 bg-indigo-900 bg-opacity-10' : ''}`}>
            ...
            <ul className={`space-y-3 mt-2 text-gray-700 dark:text-gray-400 dark:bg-gray-700 dark:bg-opacity-30 p-4 rounded-lg font-nunito ${highlightBorder ? 'border-2 border-indigo-700 bg-indigo-700 bg-opacity-10' : ''}`}>
